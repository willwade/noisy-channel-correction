...
Note: For a more comprehensive evaluation with support for synthetic conversations
and additional features, consider using conversation_level_evaluation.py with
...
def create_corrector(
    ...
    Args:
        method: Correction method ('baseline', 'context-aware', etc.)
    ...
    if method in ["context-aware", "combined"] and os.path.exists(
        word_ngram_model_path
    ):
...
def compare_methods(
    ...
    for method in methods:
        ...
        for conversation_id in conversation_ids:
            ...
            method_results["metrics"]["conversation_accuracies"].append(
                results_for_conversation["metrics"]["accuracy"]
            )

            # Update position-based metrics
            for position, accuracy in results_for_conversation["metrics"][
                "accuracy_by_position"
            ].items():
                method_results["metrics"]["accuracy_by_position"][position].append(
                    accuracy
                )

            # Update speaker-based metrics
            for speaker, accuracy in results_for_conversation["metrics"][
                "accuracy_by_speaker"
            ].items():
                method_results["metrics"]["accuracy_by_speaker"][speaker].append(
                    accuracy
                )

            # Update trend metrics
            method_results["metrics"]["accuracy_trend"].extend(
                results_for_conversation["metrics"]["accuracy_trend"]
            )

        # Calculate overall metrics
        if method_results["metrics"]["conversation_accuracies"]:
            method_results["metrics"]["overall_accuracy"] = sum(
                method_results["metrics"]["conversation_accuracies"]
            ) / len(method_results["metrics"]["conversation_accuracies"])

            # Calculate average accuracy by position
            for position, accuracies in method_results["metrics"][
                "accuracy_by_position"
            ].items():
                if accuracies:
                    method_results["metrics"]["accuracy_by_position"][position] = sum(
                        accuracies
                    ) / len(accuracies)

            # Calculate average accuracy by speaker
            for speaker, accuracies in method_results["metrics"][
                "accuracy_by_speaker"
            ].items():
                if accuracies:
                    method_results["metrics"]["accuracy_by_speaker"][speaker] = sum(
                        accuracies
                    ) / len(accuracies)
...
def visualize_method_comparison(
    ...
    if "overall_accuracy" in results["comparison"]:
        ...
        accuracies = [
            results["comparison"]["overall_accuracy"][method] for method in methods
        ]
    ...
    if "accuracy_by_position" in results["comparison"]:
        positions = sorted(
            int(pos) for pos in results["comparison"]["accuracy_by_position"].keys()
        )
        ...
        for method in methods:
            accuracies = [
                results["comparison"]["accuracy_by_position"][str(pos)].get(method, 0)
        ...
        if output_dir:
            ...
            output_path = os.path.join(output_dir, "position_comparison.png")
            logger.info(f"Saved position comparison visualization to {output_path}")
        else:
            plt.show()
    ...
    if "processing_time" in results["comparison"]:
        ...
        if output_dir:
            ...
            logger.info(f"Saved time comparison visualization to {output_path}")
        else:
            plt.show()
