...
class WordNGramModel:
    ...
    def train(self, text: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Word n-gram model trained successfully. Vocabulary size: {len(self.vocabulary)}"
            )
            return True
    ...
    def update(self, text: str) -> bool:
        ...
        try:
            logger.info(
                f"Updating word n-gram model with text ({len(text)} characters)"
            )
            ...
            logger.info(
                f"Word n-gram model updated successfully. Vocabulary size: {len(self.vocabulary)}"
            )
            return True
        except Exception as e:
    ...
    def probability_with_confidence(
        ...
        """
        Calculate the probability of a word given its context, with confidence score.
        ...
        Returns:
            Tuple of (probability, confidence) where confidence is between 0 and 1
        """
        if not self.model_ready:
            return 1e-10, 0.0  # Return a small probability with zero confidence
        ...
        # Calculate probability and confidence for each n-gram order
        probs_and_confs = []

        # Start with the highest order n-gram and work down
        for order in range(min(len(context) + 1, self.max_order), 0, -1):
            ...
            context_count = sum(
                count
                for ng, count in self.ngram_counts[order].items()
                if ng[:-1] == current_context
            )

            # Calculate probability
            if context_count > 0:
                # Simple MLE probability
                prob = ngram_count / context_count

                # Calculate confidence based on context count
                # Higher counts give higher confidence
                confidence = min(1.0, context_count / 100)  # Cap at 1.0

                probs_and_confs.append((prob, confidence, order))

        # If we have no data, return a small probability with zero confidence
        if not probs_and_confs:
            return 1e-10, 0.0

        # Interpolate probabilities based on confidence
        total_weight = sum(conf for _, conf, _ in probs_and_confs)

        if total_weight > 0:
            # Weighted average of probabilities
            interpolated_prob = (
                sum(prob * conf for prob, conf, _ in probs_and_confs) / total_weight
            )

            # Overall confidence is the highest confidence we have
            max_confidence = max(conf for _, conf, _ in probs_and_confs)

            return interpolated_prob, max_confidence
        else:
            # Fall back to unigram probability if available
            if word in self.vocabulary:
                return 1.0 / len(self.vocabulary), 0.1
            else:
                return 1e-10, 0.0
    ...
    def load(self, model_path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Word n-gram model loaded from {model_path}. Vocabulary size: {len(self.vocabulary)}"
            )
            return True
        except Exception as e:
    ...
    def _kneser_ney_probability(self, word: str, context: List[str]) -> float:
        ...
        for order in range(min(len(context) + 1, self.max_order), 0, -1):
            ...
            if ngram_count > 0 and context_count > 0:
                ...
                backoff_weight = (self.discount / context_count) * len(
                    self.ngram_counts[order]
                )

                # Calculate the backoff probability
                backoff_prob = (
                    self._kneser_ney_probability(word, context[-(order - 2) :])
                    if order > 1
                    else 1.0 / len(self.vocabulary)
                )
    ...
    def _witten_bell_probability(self, word: str, context: List[str]) -> float:
        ...
        for order in range(min(len(context) + 1, self.max_order), 0, -1):
            ...
            unique_words = len(
                set(
                    ng[-1]
                    for ng in self.ngram_counts[order].keys()
                    if ng[:-1] == current_context
                )
            )
            ...
            if context_count > 0:
                ...
                # Calculate the backoff probability
                backoff_prob = (
                    self._witten_bell_probability(word, context[-(order - 2) :])
                    if order > 1
                    else 1.0 / len(self.vocabulary)
                )
